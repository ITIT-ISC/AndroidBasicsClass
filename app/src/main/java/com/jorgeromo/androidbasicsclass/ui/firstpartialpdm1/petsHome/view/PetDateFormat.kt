 ...
 */
fun formatPetDate(iso: String, withTime: Boolean = false): String = try {
    val dt = OffsetDateTime.parse(iso)
    val month = dt.month.getDisplayName(TextStyle.SHORT, Locale("es"))
    val base = "${dt.dayOfMonth} $month ${dt.year}"
    if (withTime) "$base · ${dt.format(DateTimeFormatter.ofPattern("HH:mm"))}" else base
} catch (_: Exception) {
    iso
}
